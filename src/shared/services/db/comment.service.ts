...
class CommentService {
  public async addCommentToDB(commentData: ICommentDocument): Promise<ICommentDocument> {
    const result = await CommentsModel.create(commentData);
    await PostModel.updateOne({ $inc: { commentsCount: 1 } });
    return result;
  }
